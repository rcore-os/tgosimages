...
失败只阻止下游，其他独立任务继续，最后统一返回失败。源码准备和 patch 校验当前仍在各组件内执行，尚未拆为独立节点。`platform clean` 仍按平台顺序清理，各板级 clean 使用同一工作区锁及预算，QEMU clean 保留原工作区入口。
...
- 可选 `cache_args` 是现有 `build_task` 的输入/输出/patch/工具链声明参数数组，例如 `["--input", "config", "--output", "objects/image"]`。任务名和命令由调度器传入。只有完整声明输入后才能启用；QEMU 镜像节点当前复用编译缓存和源码校验，不启用整节点跳过。
