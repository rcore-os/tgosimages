...
`build/workspaces/qemu-<arch>/`. Graph workspace locks are acquired per target
when its first task starts and held until that target finishes, so another
target's busy workspace does not block the whole graph. First board builds in these workspaces
...
uses graph-level `locks` (held for the invocation) or task-level `locks` (held
while tasks using the workspace are pending or running). Lock sets are acquired
in sorted order and not inherited by children. `memory_mb` is an estimate, enforced for admission
...
enabling whole-task caching. A cached task automatically consumes the declared
outputs of its directly cached dependencies. Changed dependency contents invalidate
the consumer; if a producer reruns but emits identical contents, the consumer may
still hit.
...
### Adding a compiled component

Add a new Linux, RTOS, application or firmware build as one graph node with an
owned workspace and explicit outputs; do not start another background function
from an `all` handler.

1. Choose a unique task ID, an isolated source/object workspace and final paths
   below `IMAGES/<platform>/<component>/`.
2. Implement an independently executable function in the appropriate
   `scripts/platform/<platform>.sh` or `scripts/os/<component>.sh`. Use the shared
   `build_make`, `build_cmake`, `build_cargo` or `build_scons` adapter, return nonzero
   on failure, and create every declared output before returning success.
3. For an existing board, add the function to `components` in
   `scripts/lib/platform-tasks.json`, then declare data edges in `deps` and graph-local
   exclusion keys in `resources`. A QEMU OS also updates target selection in
   `scripts/platform/qemu.sh`, the guest-output map and `QEMU_OS_CACHE` in
   `qemu-graph.py`. Other flows use `phase_task` or `BuildPipeline` in their graph
   adapter instead of adding another scheduler.
4. Declare every cache input: scripts, configuration, fixed source URL/ref or a
   prepared source tree, patches, relevant environment, tools and all outputs. If
   clone/checkout remains inside the build node, record its fixed URL/ref in
   `values`; preferably split prepare/build so `sources` can inspect the prepared
   checkout. Do not cache validation-only steps, external side effects, `clean`, or
   tasks whose inputs cannot be described completely.
5. Cached consumers automatically inherit outputs from directly cached producers.
   Outputs from non-cached dependencies must be listed explicitly in `inputs`.
6. Test the initial run, identical-input hit, invalidation by source/ref/patch/config
   and dependency changes, damaged or missing outputs, failure without cache
   publication, and concurrent workspace/output ownership. Custom subtargets and
   `clean` must not reuse a full-build record.
...
Inputs and outputs must not overlap. One node owns each output. Prefer separate
workspaces and output paths for parallel variants; use a shared `resources` key only
when isolation cost is genuinely too high. Full builds may be cached; arbitrary
Make/Cargo subtargets stay uncached unless they also have complete, independent
output contracts. QEMU Linux, ArceOS, Zephyr and FreeRTOS full-build nodes follow
this contract; clean and custom subtargets do not reuse those records.

### Console colors

Shared messages use cyan/green/yellow/red for progress/success/warnings/errors,
without per-node or per-architecture identity colors. Auto mode honors `NO_COLOR`
