...
grep -Eq 'CPUQuota=[1-9][0-9]*%' "$work/scope.calls"
(
...
prepare_patched_source "$work/patched" "$base" "$work/ordered"
printf 'user edit\n' >"$work/patched/value"
if diagnostic=$(prepare_patched_source "$work/patched" "$base" "$work/ordered" 2>&1); then exit 1; fi
[[ $diagnostic == *' M value'* ]] || { printf '%s\n' "$diagnostic" >&2; exit 1; }
