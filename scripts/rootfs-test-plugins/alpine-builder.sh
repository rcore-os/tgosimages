...
prepare_builder() (
    ...
    mkdir -p "$build_root/builders/downloads" "$build_root/builders/work"
    archive_path="$build_root/builders/downloads/$archive_cache"
    build_lock_acquire lock_fd "$build_root/builders/minirootfs-$sha256.lock"
    ...
    build_lock_acquire lock_fd "$build_root/builders/image-$arch-$sha256-$manifest_sha.lock"
