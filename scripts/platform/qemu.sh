...
if test "${BASH_SOURCE[0]}" = "$0"; then
    ...
    # One scheduler owns all component tasks, including single-architecture runs.
    if [[ -z ${QEMU_GRAPH_INTERNAL:-} && $qemu_help_only == 0 && $# -ge 2 ]]; then
        case ${1:-}:${2:-} in
            aarch64:linux|aarch64:arceos|aarch64:zephyr|aarch64:freertos|aarch64:all|\
            riscv64:linux|riscv64:arceos|riscv64:all|\
...
usage() {
    ...
    printf '  * ext4 outputs contain ROOTFS_GUEST_COUNT independent guest rootfs images (default: 2, minimum: 1).\n'
    printf '  * Guest paths are zero-based: /guest/rootfs-<arch>-<type>-0.img through -(count-1).img.\n'
    printf '  * Platform payload is added only to the outer image.\n'
...
qemu_rootfs_build_step() {
    ...
    bash "${SCRIPT_DIR}/../rootfs/${rootfs_builder}.sh" \
        "${ARCH}" --out_dir "${ROOTFS_GRAPH_OUTPUT_DIR:-${ROOT_DIR}/IMAGES/rootfs}" "${ROOTFS_BUILD_ARGS[@]}"
}
...
qemu_copy_staged_rootfs() {
    local stage_dir=$1 compose_dir=$2 rootfs_builder source
    ...
    for rootfs_builder in "${ROOTFS_BUILDERS[@]}"; do
        ...
        cp --preserve=all --reflink=auto --sparse=always -- "$source" \
            "$compose_dir/rootfs-${ARCH}-${rootfs_builder}.img" || return 1
        if [[ $rootfs_builder == busybox ]]; then
            ...
            cp --preserve=all --reflink=auto --sparse=always -- "$source" \
                "$compose_dir/initramfs-${ARCH}-busybox.cpio.gz" || return 1
        fi
    done
}

qemu_validate_required_guest_files() {
    local relative
    while IFS= read -r relative; do
        [[ -z $relative ]] || [[ -f $PLATFORM_IMAGES_DIR/$relative ]] ||
            die "Required QEMU guest image not found: $PLATFORM_IMAGES_DIR/$relative"
    done <<<"${QEMU_REQUIRED_GUEST_FILES:-}"
}
...
qemu_rootfs_inject_platform_dir() (
    ...
    for rootfs_builder in "${ROOTFS_BUILDERS[@]}"; do
        ...
        _rootfs_validate_protected_outer_path "${stage_dir}/guest" "${empty_overlay_dir}" "$protected_basename" || return 1
        if [[ -n ${alpine_stage_dir} ]]; then
            _rootfs_validate_protected_outer_path "${alpine_stage_dir}/guest" "${empty_overlay_dir}" "$protected_basename" || return 1
        fi
    done

    for rootfs_builder in "${ROOTFS_BUILDERS[@]}"; do
        protected_basename="rootfs-${ARCH}-${rootfs_builder}.img"
        outer_image="${rootfs_dir}/${protected_basename}"
        ...
        case "${rootfs_builder}" in
            busybox)
                ...
                rootfs_inject_guest_stage "${rootfs_dir}/initramfs-${ARCH}-busybox.cpio.gz" "${builder_guest_dir}" || return 1
        ...
        _rootfs_validate_guest_image_set "$outer_image" "$protected_basename" || return 1
    ...
    [[ -z "${composed_rootfs_dir}" ]] || rm -rf "${composed_rootfs_dir}"
)
...
qemu_build_os_and_rootfs() {
    ...
    if [[ ${QEMU_GRAPH_INTERNAL:-} == describe ]]; then
        python3 -c 'import json,sys; json.dump(sys.argv[2:],open(sys.argv[1],"w"))' \
            "$QEMU_GRAPH_DESCRIPTION" "${parallel_steps[@]}"
        return
    elif [[ ${QEMU_GRAPH_INTERNAL:-} == execute ]]; then
        local graph_step allowed=0
        for graph_step in "${parallel_steps[@]}" qemu_rootfs_inject_platform_dir; do
            [[ $graph_step != "$QEMU_GRAPH_STEP" ]] || allowed=1
        done
        ((allowed)) || die "Unknown QEMU graph step: $QEMU_GRAPH_STEP"
        "$QEMU_GRAPH_STEP" "${BUILD_ARGS[@]}"
        return
    fi
