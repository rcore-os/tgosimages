...
rootfs_builder_prepare_test_overlays() {
    ...
    prepared_outer_overlay=$(mktemp -d "${parent}/.${owner}-outer-tests.XXXXXX") || return 1
    prepared_guest_overlay=$(mktemp -d "${parent}/.${owner}-guest-tests.XXXXXX") || {
        rm -rf -- "$prepared_outer_overlay"
        return 1
    }
    if ! "$ROOTFS_TEST_BUILD" build --arch "$arch" --rootfs "$rootfs_type" --scope outer \
        --tests "$outer_tests" --output "$prepared_outer_overlay" ||
       ! "$ROOTFS_TEST_BUILD" build --arch "$arch" --rootfs "$rootfs_type" --scope guest \
        --tests "$guest_tests" --output "$prepared_guest_overlay"; then
        rm -rf -- "$prepared_outer_overlay" "$prepared_guest_overlay"
        return 1
    fi
    ...
    if ! _rootfs_builder_normalize_overlay_seconds "$prepared_outer_overlay" ||
       ! _rootfs_builder_normalize_overlay_seconds "$prepared_guest_overlay" ||
       ! rootfs_validate_payload_tree "$prepared_outer_overlay" ||
       ! rootfs_validate_payload_tree "$prepared_guest_overlay"; then
        rm -rf -- "$prepared_outer_overlay" "$prepared_guest_overlay"
        return 1
    fi
    printf -v "$outer_var" '%s' "$prepared_outer_overlay"
