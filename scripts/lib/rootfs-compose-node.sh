...
mkdir -p -- "$output_dir"
empty=$(mktemp -d "${BUILD_WORK_DIR}/rootfs-empty.XXXXXX")
temporary="${output_dir}/.rootfs-${arch}-${rootfs_type}.compose.$$"
init_temp=
cleanup() { rm -rf -- "$empty"; rm -f -- "$temporary" "${init_temp:-}"; }
trap cleanup EXIT
...
if [[ $rootfs_type == busybox ]]; then
    init_temp="${output_dir}/.initramfs-${arch}-busybox.cpio.gz.$$"
    ...
    mkfs_publish_pair "$init_temp" "$output_dir/initramfs-${arch}-busybox.cpio.gz" \
        "$temporary" "$output_dir/rootfs-${arch}-busybox.img"
    init_temp=
    temporary=
else
